...
Cod_local VarChar(100),
 ...
 primary key(Cod_id_3D));
 
 create table Nivel(Niv_ID int not null auto_increment,
 Niv_Nome VarChar(50)not null,
 Niv_descricao VarChar(100),
 primary key(Niv_id));
 
 create table Categoria(Cat_ID Int not null auto_increment,
 Cat_Nome VarChar(50),
...
create table utilizador(
 ...
 use_Birthday datetime,
 use_Email VarChar(50), 
 use_Sexo Char(1)not null,
 use_ID int not null auto_increment,
 use_cod_id_3D char(3) not null,
 primary key(Use_ID),
 Foreign Key(use_Cod_id_3D) references codigo_postal(Cod_id_3D));

create table Personal_Trainer(Pt_id int not null auto_increment, 
 Pt_nome VarChar(50)not null,
 Pt_sexo Char(1)not null,
 Pt_email VarChar(100),
 Pt_Cod_id_3D Char(3),
 primary key(Pt_id),
 foreign key(Pt_Cod_id_3D) references codigo_postal(Cod_id_3D));
 
 create table Aula(Au_Id Int not null auto_increment,
...
************************************************************/
create table qualificado( qual_id int not null auto_increment ,
qual_local varchar(50),
 qual_horario time,
 qual_dataderealizacao datetime,
 ...
 create table pagar(pag_id int not null auto_increment,
 pag_data datetime,
 pag_quantidade int not null,
 pag_tax_pag int not null,
 pag_use_ID int not null,
 pag_Pt_id int not null,
 primary key(pag_id),
 foreign key(pag_use_ID) references utilizador(use_ID),
 foreign key(pag_Pt_id) references Personal_Trainer(Pt_id));
